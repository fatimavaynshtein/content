...
def __create_integration_instance(server, username, password, integration_name, integration_instance_name,
                                  integration_params, is_byoi, logging_manager, validate_test=True):
...
def __get_investigation_playbook_state(client, inv_id, logging_manager):
    try:
        ...
        logging_manager.exception(
            f'Failed to get investigation playbook state, error trying to communicate with demisto server'
        )
...
def __delete_integration_instance(client, instance_id, logging_manager):
    try:
...
def __delete_integrations_instances(client, module_instances, logging_manager):
    succeed = True
...
def disable_all_integrations(dem_client, logging_manager):
    """
    ...
    try:
        ...
        return
    if int(int_resp[1]) != 200:
        logging_manager.error(f'Get all integration instances failed with status code: {int_resp[1]}', real_time=True)
        return
    if 'instances' not in int_instances:
        logging_manager.info("No integrations instances found to disable all", real_time=True)
        return
    to_disable = []
    for instance in int_instances['instances']:
