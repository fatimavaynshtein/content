...
def get_latest_version_from_bucket(pack_id: str, production_bucket: Bucket) -> str:
    ...
    pack_versions_paths = [f.name for f in production_bucket.list_blobs(prefix=pack_bucket_path)]
    pack_versions = [LooseVersion(LATEST_VER_REGEX.findall(path)[0]) for path in pack_versions_paths if
...
def install_all_content_packs_for_nightly(client: demisto_client, host: str, service_account: str):
    ...
    logging.debug("Installing all content packs for nightly flow")
