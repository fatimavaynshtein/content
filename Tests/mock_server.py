...
class MITMProxy:
    ...
    def move_mock_file_to_repo(self, playbook_id):
        ...
        if not self.has_mock_file(playbook_id):
            ...
            # Move to repo folder
            self.logging_manager.info(f'Moving "{src_files}" files to "{dst_folder}" directory')
    ...
    def clean_mock_file(self, playbook_id, path=None):
        self.logging_manager.info(f'clean_mock_file was called for test "{playbook_id}"')
        path = path or self.current_folder
        problem_keys_filepath = os.path.join(path, get_folder_path(playbook_id), 'problematic_keys.json')
        self.logging_manager.info(f'problem_keys_filepath="{problem_keys_filepath}"')
        ...
        self.logging_manager.info('checking if there is data to whitewash')
        ...
        if problem_keys and needs_whitewashing:
            ...
            self.logging_manager.info(f'command to clean mockfile:\n\t{command}', print, thread_index)
            split_command = command.split()
            self.logging_manager.info('Let\'s try and clean the mockfile from timestamp data!', print, thread_index)
            try:
                ...
                self.logging_manager.success('Success!', print_color)
            ...
            self.logging_manager.info(verify_diff_start_msg)
            ...
            try:
                ...
                self.logging_manager.info(f'diff_cmd_output={diff_cmd_output}')
                if diff_cmd_output.endswith('are identical'):
                    ...
                    self.logging_manager.warning(identical_msg)
                else:
                    self.logging_manager.info('looks like the cleaning process did something!')

            except CalledProcessError as e:
                self.logging_manager.exception(f'command `{diff_cmd}` failed')

            self.logging_manager.info('Replace old mock with cleaned one.')
            ...
            self.logging_manager.info(empty_msg)
    ...
    def start(self, playbook_id, path=None, record=False):
        ...
        self.logging_manager.info(f'Proxy process up and running. Took {seconds_since_init} seconds')

        # verify that mitmdump process is listening on port 9997
        try:
            self.logging_manager.info('verifying that mitmdump is listening on port 9997')
            lsof_cmd = ['sudo', 'lsof', '-iTCP:9997', '-sTCP:LISTEN']
            lsof_cmd_output = self.ami.check_output(lsof_cmd).decode().strip()
            self.logging_manager.info(f'lsof_cmd_output={lsof_cmd_output}')
        except CalledProcessError:
            self.logging_manager.exception('No process listening on port 9997')
    ...
    def stop(self):
        ...
        # Handle logs
        self.logging_manager.debug(f'proxy outputs:\n{self.process.stdout.read()}\n{self.process.stderr.read()}')
