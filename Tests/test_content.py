...
def print_test_summary(tests_data_keeper: DataKeeperTester, is_ami: bool = True):
    ...
    logging_manager.info('\nTEST RESULTS:', real_time=True)
    logging_manager.info(f'\t Number of playbooks tested - {succeed_count + failed_count}', real_time=True)
    logging_manager.success(f'\t Number of succeeded tests - {succeed_count}', real_time=True)

    if failed_count > 0:
        logging_manager.error(f'\t Number of failed tests - {failed_count}:', real_time=True)
        logging_manager.error('\n'.join([f'\t - {playbook_id}' for playbook_id in failed_playbooks]), real_time=True)
    if rerecorded_count > 0:
        logging_manager.warning(f'\t Tests with failed playback and successful re-recording - {rerecorded_count}',
                                real_time=True)
        logging_manager.warning('\n'.join([f'\t - {playbook_id}' for playbook_id in rerecorded_tests]), real_time=True)

    if empty_mocks_count > 0:
        logging_manager.info(f'\t Successful tests with empty mock files - {empty_mocks_count}:')
        ...
        logging_manager.info(proxy_explanation, real_time=True)
        logging_manager.info('\n'.join([f'\t - {playbook_id}' for playbook_id in empty_files]), real_time=True)

    if len(skipped_integration) > 0:
        logging_manager.warning(f'\t Number of skipped integration - {len(skipped_integration):}', real_time=True)
        logging_manager.warning('\n'.join([f'\t - {playbook_id}' for playbook_id in skipped_integration]),
                                real_time=True)

    if skipped_count > 0:
        logging_manager.warning(f'\t Number of skipped tests - {skipped_count}:', real_time=True)
        logging_manager.warning('\n'.join([f'\t - {playbook_id}' for playbook_id in skipped_tests]), real_time=True)

    if unmocklable_integrations_count > 0:
        logging_manager.warning(f'\t Number of unmockable integrations - {unmocklable_integrations_count}:',
                                real_time=True)
        logging_manager.warning(
            '\n'.join([f'\t - {playbook_id} - {reason}' for playbook_id, reason in unmocklable_integrations.items()]),
            real_time=True)
...
def manage_tests(tests_settings):
    ...
    if tests_data_keeper.failed_playbooks:
        logging_manager.critical("Some tests have failed. Not destroying instances.")
        sys.exit(1)
...
def safe_unlock_integrations(integrations_details: list):
    ...
    try:
        ...
        logging_manager.exception(f'attempt to unlock integration failed for unknown reason.')
